...
WORKDIR /src
COPY ["cadll.csproj", "."]
RUN dotnet restore "./cadll.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./cadll.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./cadll.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
